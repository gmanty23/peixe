    ...
    with open(path, 'r') as f:
        for line in f:
            ...
            detections.append({
                ...
                'area': area,
                'track_id': None,
                'start_frame': None  # añadimos esto para comparar longevidad
            })
    return detections
...
def cargar_detecciones_por_frame(labels_dir, image_size=1024):
    ...
    for idx, path in enumerate(txt_files):
        ...
        # anotar frame de inicio para cada detección
        for d in detections:
            d['start_frame'] = idx
...
def conectar_nodos_hungaro(grafo, pesos, umbral_coste=np.inf):
    frames = sorted(grafo.keys())
    track_id_counter = 0

    # Inicializar track_id en el primer frame
    nodos_f0 = grafo[frames[0]]
    for nodo in nodos_f0:
        nodo["track_id"] = track_id_counter
        track_id_counter += 1
    ...
    for i in range(len(frames) - 1):
        ...
        for r, n1 in enumerate(nodos_f1):
            for c, n2 in enumerate(nodos_f2):
                cost_matrix[r, c] = calcular_coste_conexion(n1, n2, pesos)
        ...
        for r, c in zip(row_ind, col_ind):
            ...
            if coste <= umbral_coste:
                ...
                if n1["track_id"] is None:
                    n1["track_id"] = track_id_counter
                    track_id_counter += 1
                n2["track_id"] = n1["track_id"]
                asignados_f2.add(c)

        for j, n2 in enumerate(nodos_f2):
            if j not in asignados_f2:
                n2["track_id"] = track_id_counter
                track_id_counter += 1
...
def eliminar_duplicados(detecciones, iou_thresh=0.85, dist_thresh=15):
    keep = []
    eliminados = set()
    for i in range(len(detecciones)):
        ...
        for j in range(i + 1, len(detecciones)):
            ...
            if iou > iou_thresh and dist < dist_thresh:
                # Conservar la de menor área, pero heredar el ID más longevo
                if d1['area'] < d2['area']:
...
if __name__ == "__main__":
    ...
    pesos = (0.85, 0.05, 0.05, 0.05)  # α, β, γ, δ
    umbral_coste = 100  
    conectar_nodos_hungaro(grafo_temporal, pesos, umbral_coste)
    ...
    guardar_frames_con_ids(
        ...
        path_salida="/home/gmanty/code/output_20s/prueba_salida_grafo",
        image_size=1024
    )
