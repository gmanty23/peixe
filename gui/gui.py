...
class GUI(QWidget):
    ...
    def __init__(self, controller, cfg: DictConfig) -> None:
        ...
        self.fuse_masks_button = QPushButton('Fuse masks')               #NEW
        self.fuse_masks_button.clicked.connect(self.on_fuse_masks)       #NEW
        ...
        import_area.addWidget(self.fuse_masks_button)  #NEW
    ...
    def fuse_masks(self, images_path, masks_path):
        ...
        with Pool(num_threads) as p:
            ...
            args = [(file, images_path, masks_path, output_path) for file in files]
            for _ in tqdm(p.starmap(process_img, args), total=len(files), desc="Processing Images", unit="img"):
                pass

def process_img(file, images_path, masks_path, output_path):
        
    color_nuevo = (255, 0, 255)  # Color as different as it can be from the fish tank  
    ...
    if img is None or mask is None:
        return f"Error con el archivo {file}, saltando..."
        
    binary_mask = mask > 0
    img[binary_mask] = color_nuevo
